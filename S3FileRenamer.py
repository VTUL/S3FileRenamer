...
#   corrected_name = re.sub("|".join(found_bad_characters), "", old_name)
    ...
    while not done:
        if os.path.exists(os.path.join(root_path, check_name)):
            n += 1
    ...
    if rename_do:
        print("Renaming {} to {}".format(os.path.join(root_path, old_name), os.path.join(root_path, corrected_name)))
        os.rename(os.path.join(root_path, old_name), os.path.join(root_path, corrected_name))
    else:
...
if __name__ == "__main__":
    ...
    log_file = open(os.path.join(log_directory, "rename.log"), "w")
    ...
    for root, dirs, files in os.walk(path, topdown=False):
        for name in files:
            ...
            if found is True:
                ...
                log_line = "{} || {} || {}\n".format(" ".join(characters_found), os.path.join(root, name), os.path.join(root, new_name))
                print(log_line)
